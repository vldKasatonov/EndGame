...
void mx_draw_stars(int space, int star, t_game_textures *textures, Vector2 circle) {
    ...
    DrawTexturePro(textures->star, src_star,
                   (Rectangle){star_rec.x + space, star_rec.y,
				   			   star_rec.width, star_rec.height},
               	   (Vector2){0, 0}, 0.0f, star_1);
    		   
    DrawTexturePro(textures->star, src_star,
                   (Rectangle){circle.x - 36 + space, star_rec.y - 65,
                               star_rec.width, star_rec.height },
    		   	   (Vector2){0, 0}, 0.0f, star_2);
    		   
    DrawTexturePro(textures->star, src_star,
                   (Rectangle){star_rec.x + 175 + space, star_rec.y,
                               star_rec.width, star_rec.height },
...
void mx_draw_text_score(int space, Rectangle level_rec, Vector2 circle,
    ...
    DrawTextEx(mx_get_custom_font(), time,
    		   (Vector2){text_time_pos.x + space, text_time_pos.y},
               40, 3, WHITE);
}
...
void mx_draw_level(int space, int star, t_game_textures *textures,
    ...
    DrawTexturePro(map_image, src_image,
                   (Rectangle){small_rec.x + space, small_rec.y,
                               small_rec.width, small_rec.height},
	    		   (Vector2){0, 0}, 0.0f, WHITE);
    		   
    DrawRectangleRounded((Rectangle){small_rec.x + space, small_rec.y,
    				      			 small_rec.width, small_rec.height},
    				     small_corner, segments,
                         custom_colors.tonedreview_color);
    
    DrawCircleV((Vector2){circle.x + space, circle.y},
                50, custom_colors.circle_color);
    DrawCircleLinesV((Vector2) {circle.x + space, circle.y},
                     50, custom_colors.circle_stroke_color);
    ...
    DrawTexturePro(textures->play_button, src_play_button,
    		   	   (Rectangle){play_button_rec.x + space,
                               play_button_rec.y,
                               play_button_rec.width,
                               play_button_rec.height},
...
Rectangle mx_draw_back_button(t_game_textures *textures) {
	...
	Rectangle back_rect = {back_button.x,
                           back_button.y + (button_height - icon_height) / 2,
                           icon_width, icon_height};

	DrawTexturePro(textures->arrow,
                   (Rectangle){0, 0, textures->arrow.width,
                               textures->arrow.height},
                   back_rect, (Vector2){0, 0}, 0.0f, WHITE);
...
Rectangle mx_draw_settings_icon(t_game_textures *textures) {
	...
	DrawTexturePro(textures->settings,
                   (Rectangle){0, 0, textures->settings.width,
                               textures->settings.height},
