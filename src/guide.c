...
void mx_render_controls(t_game_textures *textures) {
    ...
    DrawTextEx(mx_get_custom_font(), "PAGE 0", text_pos_page,
               70, 3, WHITE);
    mx_draw_arrow(textures->right_arrow, right_arrow_rec);
    DrawTexturePro(textures->controls, (Rectangle) { 0, 0,
                   textures->controls.width,
                   textures->controls.height }, (Rectangle) { 154,
                   120, 1244, 700 }, (Vector2) { 0, 0 }, 0.0f, WHITE);
    check_collision(mouse, &cursor_changed, back_button, previous_guide_state);
