...
static void update_gameplay(t_player* player_data, t_surface* surfaces, const int surface_count) {
    ...
    if (IsKeyDown(KEY_W))
        new_pos.y -= player_config.speed;
    if (IsKeyDown(KEY_S))
        new_pos.y += player_config.speed;
    if (IsKeyDown(KEY_A))
        new_pos.x -= player_config.speed;
    if (IsKeyDown(KEY_D))
        new_pos.x += player_config.speed;
...
void mx_render_gameplay(t_player* player_data, t_game_textures* textures,
                        bool* is_popup_open, int* served_counter, int max_served) {
    Rectangle player = {player_data->position.x,
                        player_data->position.y,
                        player_config.player_width,
    ...
    for (int i = 0; i < surface_count; i++) {
        if (CheckCollisionRecs(player, surfaces[i].rect)) {
            if (surfaces[i].type != NONE)
                DrawTexturePro(textures->f_button,
                               (Rectangle){0, 0, textures->f_button.width,
    ...
    DrawTexturePro(textures->pause,
                   (Rectangle){0, 0, textures->pause.width,
                               textures->pause.height},
